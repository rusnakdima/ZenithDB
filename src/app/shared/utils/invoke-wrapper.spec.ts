...
describe('invoke-wrapper utilities', () => {
  ...
  describe('invokeWithAbortHandling', () => {
    ...
    it('should throw on AbortError with generic message', async () => {
      ...
      await expect(invokeWithAbortHandling(invokeFn, 'context', mockErrorHandler as any)).rejects.toThrow('Operation cancelled');
    });
    ...
    it('should call errorHandler and rethrow on other errors', async () => {
      ...
      await expect(invokeWithAbortHandling(invokeFn, 'context', mockErrorHandler as any)).rejects.toThrow('network error');
  ...
  describe('invokeWithAbortHandlingOrDefault', () => {
    it('should return result on success', async () => {
      ...
      const result = await invokeWithAbortHandlingOrDefault(invokeFn, 'context', mockErrorHandler as any, 'default');
    ...
    it('should call errorHandler and rethrow on other errors', async () => {
      ...
      await expect(invokeWithAbortHandlingOrDefault(invokeFn, 'context', mockErrorHandler as any, 'default')).rejects.toThrow('network error');
