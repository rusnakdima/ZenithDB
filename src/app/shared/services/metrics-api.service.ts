...
export class MetricsApiService extends CacheService {
  ...
  async fetchMetrics(): Promise<SystemMetrics> {
    this.appLogger.debug("[METRICS_API]", "fetchMetrics started");
  ...
  async fetchMetricsWithRefresh(): Promise<SystemMetrics> {
    this.appLogger.debug("[METRICS_API]", "fetchMetricsWithRefresh started");
  ...
  invalidateMetrics(): void {
    this.appLogger.debug("[METRICS_API]", "invalidateMetrics called");
  ...
  private async fetchMetricsInternal(): Promise<SystemMetrics> {
    this.appLogger.debug("[METRICS_API]", "fetchMetricsInternal started");
    ...
    return this.getOrFetch("system_metrics", fetchFn, this.METRICS_TTL_MS).then((result) => {
      this.appLogger.debug("[METRICS_API]", "fetchMetricsInternal completed");
      return result;
    });
  }
