...
export class DatabaseSelectorComponent {
  ...
  startEdit(dbName: string) {
    ...
    logger.debug("[DATABASE_SELECTOR]", "Started editing database", { dbName });
  }
  ...
  saveEdit() {
    ...
    if (this.selectedDatabases().includes(newName)) {
      logger.warn("[DATABASE_SELECTOR]", "Duplicate database name", { newName });
      this.cancelEdit();
      return;
    }

    logger.info("[DATABASE_SELECTOR]", "Database renamed", { oldName, newName });
  ...
  remove(dbName: string) {
    logger.info("[DATABASE_SELECTOR]", "Database removed", { dbName });
  ...
  addManual() {
    ...
    if (name && !this.selectedDatabases().includes(name)) {
      logger.info("[DATABASE_SELECTOR]", "Manual database added", { name });
