import {
  mockConnectionSummaries,
  mockConnectionConfigResult,
  ...
  mockRawResult,
  mockSystemMetrics,
} from "../shared/test-utils/mock-data";
...
describe("ApiProvider", () => {
  ...
  beforeEach(() => {
    ...
    apiProvider = {
      ...
      listConnections: async () => {
        const connections = await mockInvoke("list_connections", {
          options: { signal: mockCancellation.createAbortSignal() },
        });
        mockDataStore.updateConnections(connections);
        return connections;
      },
      getConnection: async (id: string) =>
        mockInvoke("get_connection", {
          id,
          options: { signal: mockCancellation.getFastAbortSignal() },
        }),
      testConnectionStatus: async (id: string) => {
        try {
          return await mockInvoke("test_connection_status", {
            id,
            options: { signal: mockCancellation.createAbortSignal() },
          });
        } catch (e) {
      ...
      saveConnection: async (config: any) => {
        const id = await mockInvoke("save_connection", {
          config,
          options: { signal: mockCancellation.createAbortSignal() },
        });
      ...
      deleteConnection: async (id: string) => {
        await mockInvoke("delete_connection", {
          id,
          options: { signal: mockCancellation.createAbortSignal() },
        });
      ...
      testConnection: async (config: any) => {
        try {
          return await mockInvoke("test_connection", {
            config,
            options: { signal: mockCancellation.createAbortSignal() },
          });
        } catch (e) {
      ...
      listCollections: async (connId: string, dbName?: string) => {
        const result = await mockInvoke("list_collections", {
          connId,
          dbName,
          options: { signal: mockCancellation.createAbortSignal() },
        });
      ...
      createDatabase: async (connId: string, name: string) => {
        await mockInvoke("create_database", {
          connId,
          name,
          options: { signal: mockCancellation.createAbortSignal() },
        });
      ...
      describeCollection: async (connId: string, collection: string) => {
        return mockInvoke("describe_collection", {
          connId,
          collection,
          options: { signal: mockCancellation.createAbortSignal() },
        });
      ...
      getCollectionStats: async (connId: string, collection: string) => {
        return mockInvoke("get_collection_stats", {
          connId,
          collection,
          options: { signal: mockCancellation.createAbortSignal() },
        });
      ...
      queryData: async (connId: string, collection: string, params: any) => {
        const result = await mockInvoke("query_data", {
          connId,
          collection,
          query: params,
          options: { signal: mockCancellation.createAbortSignal() },
      ...
      createCollection: async (connId: string, name: string) => {
        await mockInvoke("create_collection", {
          connId,
          name,
          options: { signal: mockCancellation.createAbortSignal() },
        });
        await mockInvoke("list_collections", {
          connId,
          options: { signal: mockCancellation.createAbortSignal() },
        });
      },
      dropCollection: async (connId: string, name: string) => {
        await mockInvoke("drop_collection", {
          connId,
          name,
          options: { signal: mockCancellation.createAbortSignal() },
        });
      ...
      getServerVersion: async (connId: string) => {
        return mockInvoke("get_server_version", {
          connId,
          options: { signal: mockCancellation.createAbortSignal() },
        });
      },
      getSystemStatus: async () => {
        const metrics = await mockInvoke("get_system_status", {
          options: { signal: mockCancellation.createAbortSignal() },
        });
      ...
      updateConnection: async (id: string, config: any) => {
        await mockInvoke("update_connection", {
          id,
          config,
          options: { signal: mockCancellation.createAbortSignal() },
        });
      ...
      renameCollection: async (connId: string, oldName: string, newName: string) => {
        await mockInvoke("rename_collection", {
          connId,
          oldName,
          newName,
          options: { signal: mockCancellation.createAbortSignal() },
      ...
      renameDatabase: async (connId: string, oldName: string, newName: string) => {
        await mockInvoke("rename_database", {
          connId,
          oldName,
          newName,
          options: { signal: mockCancellation.createAbortSignal() },
      ...
      deleteDatabase: async (connId: string, name: string) => {
        await mockInvoke("delete_database", {
          connId,
          name,
          options: { signal: mockCancellation.createAbortSignal() },
        });
  ...
  describe("testConnectionStatus", () => {
    ...
    it("should handle abort errors", async () => {
      ...
      await expect(
        apiProvider.testConnectionStatus("12345678-1234-1234-1234-123456789012")
  ...
  describe("listCollections", () => {
    it("should return collections list", async () => {
      const mockResult = {
        collections: [{ name: "users", count: 10 }],
        has_more: false,
        total_count: 1,
      };
