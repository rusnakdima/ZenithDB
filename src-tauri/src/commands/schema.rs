...
#[tauri::command]
pub async fn list_databases(connId: &str) -> Result<ResponseModel, ResponseModel> {
  let timer = DataflowTimer::new("list_databases");
  let result = (|| async {
    validate_conn_id(connId).map_err(|e| ResponseModel::error(e))?;
    let entry = get_connection_entry(connId)
      .await
    ...
    match &entry.config.config {
      ConnectionConfigEnum::Json { path: _, .. } => {
        ...
        let databases = provider
          ...
          .await
          .map_err(|e| ResponseModel::error(e.to_string()))?;
        Ok::<ResponseModel, ResponseModel>(ResponseModel::success(databases))
      }
      ConnectionConfigEnum::Sqlite { path, .. } => {
        let db_name = std::path::Path::new(path)
          ...
          .unwrap_or("database")
          .to_string();
        Ok::<ResponseModel, ResponseModel>(ResponseModel::success(vec![DatabaseMeta::from_name(
          &db_name,
        )]))
      }
      ConnectionConfigEnum::Redis { .. } => Ok::<ResponseModel, ResponseModel>(
        ResponseModel::success(vec![DatabaseMeta::from_name("default")]),
      ),
      ConnectionConfigEnum::Mongo { uri, .. } => {
        let provider =
          crate::commands::provider::get_or_create_mongo_provider(connId, uri, "admin")
            .await
            .map_err(|e| ResponseModel::error(e.to_string()))?;
        let result = provider
        ...
        Ok::<ResponseModel, ResponseModel>(ResponseModel::success(dbs))
      }
      ConnectionConfigEnum::Postgres { uri, .. } => {
        let provider = crate::commands::provider::get_or_create_postgres_provider(connId, uri)
          .await
          .map_err(|e| ResponseModel::error(e.to_string()))?;
        let result = provider
          .execute_raw(
          ...
          .await
          .map_err(|e| ResponseModel::error(e.to_string()))?;
        Ok::<ResponseModel, ResponseModel>(ResponseModel::success(parse_database_rows(
          &result.rows,
        )))
      }
      ConnectionConfigEnum::MySql { uri, .. } => {
        let provider = crate::commands::provider::get_or_create_mysql_provider(connId, uri)
          .await
          .map_err(|e| ResponseModel::error(e.to_string()))?;
        let result = provider
...
#[tauri::command]
pub async fn create_database(connId: &str, name: &str) -> Result<ResponseModel, ResponseModel> {
...
#[tauri::command]
pub async fn delete_database(connId: &str, name: &str) -> Result<ResponseModel, ResponseModel> {
...
pub async fn list_databases_for_uri(
  ...
  let timer = DataflowTimer::new("list_databases_for_uri");
  let result = (|| async {
    if uri.is_empty() {
