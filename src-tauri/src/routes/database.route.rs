...
use crate::routes::types::DatabaseMeta;
  ...
  let result = (|| async {
    ...
    match &entry.config.config {
      ...
      ConnectionConfigEnum::Mongo { uri, .. } => {
        let provider = crate::routes::provider::create_mongo_provider(uri, &name)
          .await
          .map_err(|e| ResponseModel::error(e.to_string()))?;
        provider
...
pub async fn database_list(
  ...
  let result = match &entry.config.config {
    ...
    ConnectionConfigEnum::Mongo { uri, .. } => {
      match crate::routes::provider::get_or_create_mongo_provider(&connection_id, uri, "admin")
        .await
      {
    ...
    ConnectionConfigEnum::Postgres { uri, .. } => {
      match crate::routes::provider::get_or_create_postgres_provider(&connection_id, uri).await {
        Ok(provider) => match provider.list_databases().await.map_err_string() {
          Ok(db_names) => {
            ResponseModel::success(list_databases_with_pagination(db_names, offset, limit))
          }
    ...
    ConnectionConfigEnum::MySql { uri, .. } => {
      match crate::routes::provider::get_or_create_mysql_provider(&connection_id, uri).await {
        Ok(provider) => match provider.list_databases().await.map_err_string() {
          Ok(db_names) => {
            ResponseModel::success(list_databases_with_pagination(db_names, offset, limit))
          }
